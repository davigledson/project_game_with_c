...
//   }
  ...
  while (1) {
        ...
        // Escolhe uma vitima aleat�ria
        int victim_index;
        do {
            victim_index = numero_aleatorio(num_players);
             //printf("------------%d --------\n",victim_index);
        } while (victim_index == impostor_index); // Garante que a v�tima n�o seja o impostor
        ...
        printf("Quem � o impostor? [digite de 0 at� %d]: ", num_players - 1);
        //printf("%s",boneco_among_us);
        while (scanf("%d", &user_input) != 1 || user_input < 0 || user_input >= num_players) { // Verifica��o de entrada v�lida
            while (getchar() != '\n'); // Limpa o buffer de entrada
            printf("Entrada inv�lida. Tente novamente. Quem � o impostor? [digite de 0 at� %d]: ", num_players - 1);
        }
